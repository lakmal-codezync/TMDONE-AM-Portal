...
// URL: #/home/reports -> Fulfillment Report tab
...
const REPORT_KEYWORD = /fulfillment|fullfilment/i;

/** @param {import('@playwright/test').Page} page */
async function goToFulfillmentReport(page) {
  ...
  await expect(page.locator('body'), 'Fulfillment report should be active or visible as the default report.').toContainText(
    REPORT_KEYWORD,
    { timeout: 15000 }
  );
}
...
async function expectReportResult(page) {
  ...
  await expect(emptyState, 'Fulfillment report should show a table or an empty-state message.').toBeVisible({
    timeout: 10000,
  });
}

test.describe.serial('Fulfillment Report', () => {
  test('FR-01: Navigate to Fulfillment Report tab', async ({ page }) => {
    await goToFulfillmentReport(page);
    const body = await page.innerText('body');
    expect(body.length).toBeGreaterThan(100);
    expect(body).toMatch(REPORT_KEYWORD);
  });

  test('FR-02: Select a date range automatically', async ({ page }) => {
    await goToFulfillmentReport(page);
    const selected = await selectDate(page);
    expect(selected, 'Fulfillment report should allow date selection or have visible date inputs.').toBeTruthy();
    await expect(page.locator('body')).toContainText(REPORT_KEYWORD);
  });

  test('FR-03: Select a store from dropdown automatically', async ({ page }) => {
    await goToFulfillmentReport(page);
  ...
  test('FR-04: Search icon/button triggers data load', async ({ page }) => {
    await goToFulfillmentReport(page);
    await selectDate(page);
    await selectStoreIfAvailable(page);
  ...
  test('FR-05: Download/Export button triggers file download', async ({ page }) => {
    await goToFulfillmentReport(page);
    await selectDate(page);
    await selectStoreIfAvailable(page);
    await clickSearch(page);
    ...
    if (download) {
      expect(download.suggestedFilename()).toMatch(REPORT_KEYWORD);
    }
  });
});
