...
// URL: #/home/reports -> Menu Optimization Report tab
...
const REPORT_KEYWORD = /menu optimization/i;

/** @param {import('@playwright/test').Page} page */
async function goToMenuOptimizationReport(page) {
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    ...
    if (await tab.isVisible().catch(() => false)) {
      ...
      await expect(page.locator('body'), 'Menu Optimization report tab should be active.').toContainText(REPORT_KEYWORD, {
        timeout: 15000,
      });
      return;
    }
  ...
  throw new Error('Menu Optimization report tab was not found.');
}
...
async function expectReportResult(page) {
  ...
  await expect(emptyState, 'Menu Optimization report should show a table or an empty-state message.').toBeVisible({
    timeout: 10000,
  });
}

test.describe.serial('Menu Optimization Report', () => {
  test('MO-01: Navigate to Menu Optimization Report tab', async ({ page }) => {
    await goToMenuOptimizationReport(page);
    const body = await page.innerText('body');
    expect(body.length).toBeGreaterThan(100);
    expect(body).toMatch(REPORT_KEYWORD);
  });

  test('MO-02: Select a date range automatically', async ({ page }) => {
    await goToMenuOptimizationReport(page);
    const selected = await selectDate(page);
    expect(selected, 'Menu Optimization report should allow date selection or have visible date inputs.').toBeTruthy();
    await expect(page.locator('body')).toContainText(REPORT_KEYWORD);
  });

  test('MO-03: Select a store from dropdown automatically', async ({ page }) => {
    await goToMenuOptimizationReport(page);
  ...
  test('MO-04: Search icon/button triggers data load', async ({ page }) => {
    await goToMenuOptimizationReport(page);
    await selectDate(page);
    await selectStoreIfAvailable(page);
  ...
  test('MO-05: Download/Export button triggers file download', async ({ page }) => {
    await goToMenuOptimizationReport(page);
    await selectDate(page);
    await selectStoreIfAvailable(page);
    await clickSearch(page);
    ...
    if (download) {
      expect(download.suggestedFilename()).toMatch(/menu|optimization/i);
    }
  });
});
