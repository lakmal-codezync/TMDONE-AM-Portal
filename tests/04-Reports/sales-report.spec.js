...
// URL: #/home/reports -> Sales Report tab
...
const REPORT_KEYWORD = /sales/i;

/** @param {import('@playwright/test').Page} page */
async function goToSalesReport(page) {
  ...
  const tabLocator = () =>
    page.locator('.mat-tab-label, [role="tab"], .mat-mdc-tab').filter({ hasText: /sales report|^sales$/i }).first();
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    ...
    if (await tab.isVisible().catch(() => false)) {
      ...
      await expect(page.locator('body'), 'Sales report tab should be active.').toContainText(REPORT_KEYWORD, {
        timeout: 15000,
      });
      return;
    }
  ...
  throw new Error('Sales report tab was not found.');
}
...
async function expectReportResult(page) {
  ...
  await expect(emptyState, 'Sales report should show a table or an empty-state message.').toBeVisible({
    timeout: 10000,
  });
}

test.describe.serial('Sales Report', () => {
  test('SR-01: Navigate to Sales Report tab', async ({ page }) => {
    await goToSalesReport(page);
    const body = await page.innerText('body');
    expect(body.length).toBeGreaterThan(100);
    expect(body).toMatch(REPORT_KEYWORD);
  });

  test('SR-02: Select a date range automatically', async ({ page }) => {
    await goToSalesReport(page);
    const selected = await selectDate(page);
    expect(selected, 'Sales report should allow date selection or have visible date inputs.').toBeTruthy();
    await expect(page.locator('body')).toContainText(REPORT_KEYWORD);
  });

  test('SR-03: Select a store from dropdown automatically', async ({ page }) => {
    await goToSalesReport(page);
  ...
  test('SR-04: Search icon/button triggers data load', async ({ page }) => {
    await goToSalesReport(page);
    await selectDate(page);
    await selectStoreIfAvailable(page);
  ...
  test('SR-05: Download/Export button triggers file download', async ({ page }) => {
    await goToSalesReport(page);
    await selectDate(page);
    await selectStoreIfAvailable(page);
    await clickSearch(page);
    ...
    if (download) {
      expect(download.suggestedFilename()).toMatch(/sales/i);
    }
  });
});
