...
// URL: #/home/reports -> Orders Count Report tab
...
const REPORT_KEYWORD = /orders count/i;

/** @param {import('@playwright/test').Page} page */
async function goToOrdersCountReport(page) {
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    ...
    if (await tab.isVisible().catch(() => false)) {
      ...
      await expect(page.locator('body'), 'Orders Count report tab should be active.').toContainText(REPORT_KEYWORD, {
        timeout: 15000,
      });
      return;
    }
  ...
  throw new Error('Orders Count report tab was not found.');
}
...
async function expectReportResult(page) {
  ...
  await expect(emptyState, 'Orders Count report should show a table or an empty-state message.').toBeVisible({
    timeout: 10000,
  });
}

test.describe.serial('Orders Count Report', () => {
  test('OC-01: Navigate to Orders Count Report tab', async ({ page }) => {
    await goToOrdersCountReport(page);
    const body = await page.innerText('body');
    expect(body.length).toBeGreaterThan(100);
    expect(body).toMatch(REPORT_KEYWORD);
  });

  test('OC-02: Select a date range automatically', async ({ page }) => {
    await goToOrdersCountReport(page);
    const selected = await selectDate(page);
    expect(selected, 'Orders Count report should allow date selection or have visible date inputs.').toBeTruthy();
    await expect(page.locator('body')).toContainText(REPORT_KEYWORD);
  });

  test('OC-03: Select a store from dropdown automatically', async ({ page }) => {
    await goToOrdersCountReport(page);
  ...
  test('OC-04: Search icon/button triggers data load', async ({ page }) => {
    await goToOrdersCountReport(page);
    await selectDate(page);
    await selectStoreIfAvailable(page);
  ...
  test('OC-05: Download/Export button triggers file download', async ({ page }) => {
    await goToOrdersCountReport(page);
    await selectDate(page);
    await selectStoreIfAvailable(page);
    await clickSearch(page);
    ...
    if (download) {
      expect(download.suggestedFilename()).toMatch(/orders|count/i);
    }
  });
});
